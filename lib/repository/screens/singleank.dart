...
import '../../bloc/singleank/singleank_bloc.dart';
import '../../bloc/singleank/singleank_event.dart';
import '../../bloc/singleank/singleank_state.dart';
import '../screens/walletscreen.dart';

class Singleank extends StatelessWidget {
  final String mid;
  final String cmid;
  final String marketName;

  const Singleank({super.key, required this.mid, required this.cmid,required this.marketName});
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (_) =>
      SingleankBloc()..add(LoadSingleank(mid: mid, cmid: cmid)),
      child: BlocBuilder<SingleankBloc, SingleankState>(
        builder: (context, state) {
          if (state is SingleankLoading) {
            return const Scaffold(
              body: Center(child: CircularProgressIndicator()),
            );
          } else if (state is SingleankLoaded) {
            return Scaffold(
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: _inputField(
                              "Single Digit", "Enter Digit", digitController,
                              singleDigitOnly: true),
                        ),
                        const SizedBox(width: 10),
                        Expanded(
                          child: _inputField(
                              "Points", "Enter Points", pointsController),
                        ),
                      ],
                    ),
                    const SizedBox(height: 20),
                    ElevatedButton(
                      onPressed: () {
                        if (digitController.text.isNotEmpty &&
                          ...
                          context.read<SingleankBloc>().add(AddBid(
          ...
          } else if (state is SingleankError) {
            return Scaffold(
